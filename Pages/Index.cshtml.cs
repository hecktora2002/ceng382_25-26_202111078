...
namespace MyApp.Pages
{
    public class IndexModel : PageModel
    {
        ...
        public IActionResult OnPostExportJson(string exportType, List<string> SelectedColumns)
        {
            List<ClassInformationModel> data;

            if (exportType == "all")
            {
                // Tüm veriler, filtre yok
                data = Classes;
            }
            else if (exportType == "currentpage")
            {
                // ✅ Sadece o anki sayfayı export et (filtre uygulanmaz)
                int pageSize = 10;
                data = Classes
                    .Skip((CurrentPage - 1) * pageSize)
                    .Take(pageSize)
            ...
            {
                // (Eğer filtre kullanmak istersen burada uygulanabilir)
                data = new(); // Boş bırakabilirsin veya future için kullanılır
            }

            var json = Utils.Instance.ExportToJson(data, SelectedColumns);

            var fileName = exportType + "-page-" + CurrentPage + ".json";
